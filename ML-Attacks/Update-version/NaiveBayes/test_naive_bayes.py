...
# === Load Test Data ===
X_test = pd.read_csv("NaiveBayes_X_test.csv")
y_test = pd.read_csv("NaiveBayes_y_test.csv").squeeze()
...
print("\nNaive Bayes Multi-Class Evaluation Metrics:")
...
plt.tight_layout()
plt.savefig("naive_bayes_multiclass_confusion_matrix.png")
plt.show()
...
for i in range(len(class_labels)):
    ...
    plt.plot(fpr[i], tpr[i], label=f"{class_labels[i]} (AUC = {roc_auc_class[i]:.2f})")
...
plt.tight_layout()
plt.savefig("naive_bayes_roc_multiclass_selected.png")
plt.show()

# === Save Evaluation Metrics to TXT ===
with open("naive_bayes_evaluation_metrics.txt", "w") as f:
    f.write("Naive Bayes Multi-Class Evaluation Metrics:\n")
